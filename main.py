# main.py
"""
Streamlit app: EcoMind AI – SDG Sustainability Analyzer
...
# Page config
st.set_page_config(page_title="EcoMind AI – SDG Sustainability Analyzer", layout="wide", initial_sidebar_state="auto")
...
<div style='background:{bg};border-left:4px solid {border};padding:10px;border-radius:8px;margin-bottom:8px'>
  <div style='display:flex;align-items:center;justify-content:space-between'>
    <div style='font-weight:700;font-size:15px;color:{text_color}'>{sid} — {name}</div>
    ...
    if "reverse" in st.session_state:
        ...
        if per:
            for rev in per:
                ...
                st.subheader(f"Ideas for SDG {sdg_val} — {sdg_label}")
                ideas = rev.get("ideas", [])
                if not ideas:
...
with tab3:
    ...
    st.markdown("""
**EcoMind AI – SDG Sustainability Analyzer**

This tool helps innovators and teams quickly evaluate how a project idea aligns with the United Nations Sustainable Development Goals (SDGs). All text analysis is generated by a large language model (Google Gemini) and returned to you in-session only — nothing is persisted.
...
- The app sends the description to Gemini, which returns a structured JSON analysis:
  - A short list of the most relevant SDGs (3–5)
  - Relevance scores (0–100)
  - Brief explanations, risks, feasibility, and recommendations
- The Reverse Idea Generator lets you select an SDG and get 3–5 aligned project concepts.
